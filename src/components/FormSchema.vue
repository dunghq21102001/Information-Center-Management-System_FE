<template>
    <div class="w-full mt-4">
        <div class="w-[95%] mx-auto grid grid-cols-12 gap-3">
            <div v-for="item in schemaProp" :key="item.title"
                :class="`col-span-12 ${item.w == 1 ? 'md:col-span-6' : 'md:col-span-12'}`">
                <label>{{ item.title }}</label>
                <input v-if="item.type === 'text' || item.type === 'number'" @focus="item.focus = true"
                    @blur="item.focus = false" :type="item.type" :class="{ 'on-focus': item.focus }" v-model="item.value"
                    class="input-cus" />
                <textarea cols="1" rows="5" v-else-if="item.type === 'textarea'" @focus="item.focus = true"
                    @blur="item.focus = false" :class="{ 'on-focus': item.focus }" v-model="item.value"
                    class="input-cus"></textarea>
                <div @dragover.prevent="handleDragOver($event, item?.field)" @drop="handleDrop($event, item?.field)"
                    class="w-full h-[140px] mt-4 relative br-dash" v-else-if="item.type === 'image'">
                    <div class="absolute top-[50%] translate-y-[-50%] left-6 flex items-center">
                        <button class="btn-primary px-3 py-1">
                    ...
                    <input type="file" id="img" class="w-full h-[100px] cursor-pointer opacity-0" ref="fileInput"
        ...
        <div class="w-[95%] mx-auto flex items-center justify-end">
            ...
            <button class="btn-primary px-4 py-1" @click="submitForm">{{ btnProp }}</button>
        </div>
    </div>
</template>
